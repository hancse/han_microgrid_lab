...
USE_LIB3PMETRICS = 0
ifeq ($(strip $(TP_METRICS)), 1)
  ifeq ("$(shell dpkg -l | grep lib3pmetrics >/dev/null 2>&1 && echo 1)","1")
    USE_LIB3PMETRICS = 1
  else
    $(error ERROR Build fails for '$(MODEL)'. Set to use Triphase metrics aggregation library 'lib3pmetrics', however it is not installed)
  endif
endif
...
ifeq ($(USE_LIB3PMETRICS),1)
  EXT_CC_OPTS += -DTP_METRICS
  LDFLAGS += ${shell pkg-config --libs lib3pmetrics --silence-errors}
  TRIPHASE_INCLUDES += ${shell pkg-config --cflags lib3pmetrics --silence-errors}
endif
